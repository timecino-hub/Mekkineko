...
*   Set a custom 16-color palette from an external array (RGB565 byte-swapped).
*   @param  palette - Pointer to 16 uint16_t RGB565 values */
void LCD_Apply_Palette(const uint16_t* palette);
...
/* Blit Fullscreen Sprite
*   Fast full-screen blit. Directly packs 4-bit pixel data into the framebuffer.
*   Marks all rows dirty for a full refresh.
*   @param  sprite - 240x240 array (0-15 = colour indices, no transparency) */
void LCD_Blit_Fullscreen(const uint8_t *sprite);
